...
/**
 * Map of variantTypeName → currently selected VariantOptionsShow for that type.
 * Lets us track one selection per variant axis (e.g., Size + Color).
 */
...
export default function ProductDetailClient({
    ...
    // Variant selections — one per variant type
    ...
    useEffect(() => {
        ...
        for (const variantType of productDetails.productVariants) {
            ...
            // Try to find the option matching `initialVariantId`
    ...
    const handleSelectVariantOption = (
        ...
        // Swap image if the option has one
    ...
    const displayPrice = (): string => {
        ...
        // Sum adjustments from all selected variant options
    ...
    return (
        <div className="mx-auto px-4 sm:px-8 md:px-16 lg:px-20 xl:px-28 py-10 max-w-7xl w-full">
            <div className="flex flex-col lg:flex-row gap-10 lg:gap-14">

                {/* ─── LEFT COLUMN: Image Gallery ─── */}
                <div className="w-full lg:w-1/2 flex flex-col gap-5 lg:sticky lg:top-8 self-start">
                ...
                <div className="w-full lg:w-1/2 flex flex-col gap-8">
                    ...
                    {/* ─── Variant Selectors ─── */}
                    {hasVariants && productDetails.productVariants!.map((variant, vi) => (
                        <motion.div
                            key={vi}
                            initial={{ opacity: 0, y: 12 }}
                            animate={{ opacity: 1, y: 0 }}
                            transition={{ duration: 0.45, delay: 0.15 + vi * 0.05 }}
                        ...
                        ${isSelected
                                        ...
                                        >
                                            ...
                                            {/* Small image preview badge for options with images */}
                                            {hasImage && (
                    ...
                    {/* Divider (only show if there were variants above or we have content below) */}
                    <hr className="border-greyColor/30" />
                                    ...
                                    >
                                        ...
                                        <AnimatePresence>
                                            {openSection === section.id && (
                                                ...
                                                >
                                                    <div className="px-5 pb-5 text-tertiaryColor/80 text-sm leading-relaxed whitespace-pre-line border-t border-greyColor/20 pt-4">
                                                        {section.content}
                                                    </div>
                                                </motion.div>
                                            )}
