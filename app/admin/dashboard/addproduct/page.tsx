...
/* ─── Module-level helpers (MUST be outside page() to avoid remount on every render) ── */
        ...
        <div className='flex flex-col gap-3'>
          ...
          <div className='rounded-lg border border-greyColor overflow-hidden'>
            <button
              ...
              style={{ background: initialUIComponentState.descriptionUI ? 'rgba(232,147,71,0.08)' : 'transparent' }}
              onClick={() => dispatchInitialUIComponent({ type: 'UPDATE_FIELD', field: 'descriptionUI', value: !initialUIComponentState.descriptionUI })}
            >
              ...
              <div className='flex items-center gap-3'>
                {ProductDetailState.description?.length ? (
                  <span className='text-xs text-secondary'>{ProductDetailState.description.length} chars</span>
                ) : null}
                <svg
                  className={`w-4 h-4 text-secondary transition-transform duration-200 ${initialUIComponentState.descriptionUI ? 'rotate-180' : ''}`}
                  fill='none' stroke='currentColor' strokeWidth='2' viewBox='0 0 24 24'>
                  <path strokeLinecap='round' strokeLinejoin='round' d='M19 9l-7 7-7-7' />
                </svg>
              </div>
            </button>
            {initialUIComponentState.descriptionUI && (
              <div className='border-t border-greyColor'>
                <textarea
                  ...
                  onChange={(e) => {
                    dispatchProductDetailCreate({ type: 'UPDATE_DESCRIPTION', payload: e.target.value });
          ...
          <div className='rounded-lg border border-greyColor overflow-hidden'>
            <button
              ...
              style={{ background: initialUIComponentState.specificationsUI ? 'rgba(232,147,71,0.08)' : 'transparent' }}
              onClick={() => dispatchInitialUIComponent({ type: 'UPDATE_FIELD', field: 'specificationsUI', value: !initialUIComponentState.specificationsUI })}
            >
              <div className='flex items-center gap-2'>
                <svg className='w-4 h-4 text-primaryColor' fill='none' stroke='currentColor' strokeWidth='2' viewBox='0 0 24 24'>
              ...
              <div className='flex items-center gap-3'>
                {ProductDetailState.specifications?.length ? (
                  <span className='text-xs text-secondary'>{ProductDetailState.specifications.length} chars</span>
                ) : null}
                <svg
                  className={`w-4 h-4 text-secondary transition-transform duration-200 ${initialUIComponentState.specificationsUI ? 'rotate-180' : ''}`}
                  fill='none' stroke='currentColor' strokeWidth='2' viewBox='0 0 24 24'>
                  <path strokeLinecap='round' strokeLinejoin='round' d='M19 9l-7 7-7-7' />
                </svg>
              </div>
            </button>
            {initialUIComponentState.specificationsUI && (
              <div className='border-t border-greyColor'>
                <textarea
                  rows={7}
                  placeholder='List specifications (weight, dimensions, material…)'
                  value={ProductDetailState.specifications || ''}
                  onChange={(e) => dispatchProductDetailCreate({ type: 'UPDATE_SPECIFICATIONS', payload: e.target.value })}
                  className='w-full bg-transparent text-sm text-primaryColor placeholder:text-secondary p-4 resize-none outline-none'
                />
              </div>
            )}
          ...
          <div className='rounded-lg border border-greyColor overflow-hidden'>
            <button
              type='button'
              className='w-full flex items-center justify-between px-4 py-3 transition-colors duration-150'
              style={{ background: initialUIComponentState.featuresUI ? 'rgba(232,147,71,0.08)' : 'transparent' }}
              onClick={() => dispatchInitialUIComponent({ type: 'UPDATE_FIELD', field: 'featuresUI', value: !initialUIComponentState.featuresUI })}
            >
              <div className='flex items-center gap-2'>
                <svg className='w-4 h-4 text-primaryColor' fill='none' stroke='currentColor' strokeWidth='2' viewBox='0 0 24 24'>
                  <path strokeLinecap='round' strokeLinejoin='round' d='M5 13l4 4L19 7' />
              ...
              <div className='flex items-center gap-3'>
                {ProductDetailState.features?.length ? (
                  <span className='text-xs text-secondary'>{ProductDetailState.features.length} chars</span>
                ) : null}
                <svg
                  className={`w-4 h-4 text-secondary transition-transform duration-200 ${initialUIComponentState.featuresUI ? 'rotate-180' : ''}`}
                  fill='none' stroke='currentColor' strokeWidth='2' viewBox='0 0 24 24'>
                  <path strokeLinecap='round' strokeLinejoin='round' d='M19 9l-7 7-7-7' />
                </svg>
              </div>
            </button>
            {initialUIComponentState.featuresUI && (
              <div className='border-t border-greyColor'>
                <textarea
                  rows={7}
                  placeholder='Highlight key product features…'
                  value={ProductDetailState.features || ''}
                  onChange={(e) => dispatchProductDetailCreate({ type: 'UPDATE_FEATURES', payload: e.target.value })}
                  className='w-full bg-transparent text-sm text-primaryColor placeholder:text-secondary p-4 resize-none outline-none'
                />
              </div>
            )}
